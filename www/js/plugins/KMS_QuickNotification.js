 ...
 *
 * @param messageOffSwitch
 * @default 0
 * @type switch
 * @desc アイテム取得やG取得時のメッセージ表示を切り替えます。（タツノコ改修）
 * 
 * @help
 *
 * ■ プラグインコマンド
...
var KMS = KMS || {};
// add tatsunoko
const dobbyTest = (window.Testtest = window.Testtest || {});
...
(function () {
    ...
    Params.messageOffSwitch = Number(pluginParams['messageOffSwitch'] || 0);
    // 通知 SE の解析
    (function () {
    ...
    // // Change Items
    var _D_Game_Interpreter_command126 = Game_Interpreter.prototype.command126;
    Game_Interpreter.prototype.command126 = function () {
        if (!$gameSwitches.value(Params.messageOffSwitch)) {
            $gameTemp.setQuickNotificationRegistrationMode(true);
            if (!$gameMessage.isBusy() &&
                $gameTemp.isQuickNotificationRegistrationMode()) {
                $gameTemp.setQuickNotificationRegistrationMode(false);
                var num = "";
                if (this._params.length > 2 && this._params[3] != "" && Number(this._params[3]) > 1) {
                ...
                var notification =
                {
                    text: icon + $dataItems[this._params[0]].name + txt,
                    faceName: '',
                    faceIndex: ''
                };
                // 通知の登録
        ...
        return _D_Game_Interpreter_command126.call(this);
    };

    // // Change Weapons
    var _D_Game_Interpreter_command127 = Game_Interpreter.prototype.command127;
    Game_Interpreter.prototype.command127 = function () {
        if (!$gameSwitches.value(Params.messageOffSwitch)) {
            ...
            if (!$gameMessage.isBusy() &&
                ...
                var notification =
                {
                    text: icon + $dataWeapons[this._params[0]].name + txt,
                    faceName: '',
                    faceIndex: ''
                };
                // 通知の登録
        ...
        return _D_Game_Interpreter_command127.call(this);
    };

    // // Change Armors
    var _D_Game_Interpreter_command128 = Game_Interpreter.prototype.command128;
    Game_Interpreter.prototype.command128 = function () {
        if (!$gameSwitches.value(Params.messageOffSwitch)) {
            ...
            if (!$gameMessage.isBusy() &&
                ...
                var notification =
                {
                    text: icon + $dataArmors[this._params[0]].name + txt,
                    faceName: '',
                    faceIndex: ''
                };
                // 通知の登録
        ...
        return _D_Game_Interpreter_command128.call(this);
    };

    // // Maney Change
    var _D_Game_Interpreter_command125 = Game_Interpreter.prototype.command125;
    Game_Interpreter.prototype.command125 = function () {
        ...
        return _D_Game_Interpreter_command125.call(this);
    };

    Game_Interpreter.prototype.getDobbyIconId = function (param) {
        return (param == "1" ? "\\I[514] " : "\\I[513] ")
    ...
    Window_QuickNotification.prototype.newPage = function (textState) {
        ...
        this.contents.blt(
            ...
            (770 / 2) - (this._allTextWidth / 2), 0, this._allTextWidth, this._allTextHeight
        );
    };
    ...
    Window_QuickNotification.prototype.updatePosition = function () {
        ...
        var xOffset = (Graphics.width / 2) - (this.width / 2);
        this.x = xOffset;
    ...
    // add tatsunoko
    dobbyTest.testtest = Window_QuickNotification;
})();