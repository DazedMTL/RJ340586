...
Scene_Base.prototype.initialize = function () {
    ...
    if ($gameTemp) {
        $gameTemp._mcursorData = [false, 0, 0, 0]
    }
...
Scene_Base.prototype.update = function () {
    ...
    if (!this._sprite_mcursor && $gameTemp && $gameTemp._mcursorData[0]) {
        this.create_menucursor()
    }
    if (this._sprite_mcursor) {
        this.update_mcursor();
    }
};
...
Scene_Base.prototype.create_menucursor = function () {
    ...
    if (String(Moghunter.mcursor_float) === "true") {
        this._mcursor_data[5] = true
    }
    if (String(Moghunter.mcursor_animated) === "true") {
        this._mcursor_data[6] = true
    }
...
Scene_Base.prototype.update_mcursor = function () {
    if (this.isBusy()) {
        this._sprite_mcursor.opacity = 0;
        // this._sprite_mcursor.opacity -= 25;
        return
    }
    ...
    if ($gameTemp._mcursorData[1] > 0) {
        ...
        this._sprite_mcursor.opacity = 255;
        // this._sprite_mcursor.opacity += 15;
    } else {
        this._sprite_mcursor.opacity = 0
    }
    ...
    this._sprite_mcursor.x = nx;
    this._sprite_mcursor.y = $gameTemp._mcursorData[3] + this._mcursor_data[1];
    // var nx                 = $gameTemp._mcursorData[2] + this._mcursor_data[0] + this._mcursor_data[2];
...
Scene_Base.prototype.set_mcursor_initial = function () {
    if (!this._sprite_mcursor.bitmap.isReady()) {
        return
    }
...
Scene_Base.prototype.update_mcursor_float = function () {
    ...
    if (this._mcursor_data[4] < 2) {
        return;
    }
    this._mcursor_data[4] = 0;
    this._mcursor_data[3] += 1;
    if (this._mcursor_data[3] < 15) {
        this._mcursor_data[2] += 1
    } else if (this._mcursor_data[3] < 30) {
        this._mcursor_data[2] -= 1
    } else {
        this._mcursor_data[2] = 0;
        this._mcursor_data[3] = 0
    }
};
...
Window_Selectable.prototype.need_set_mcursor_data = function () {
    ...
    if (this.x <= -this.width || this.x >= Graphics.boxWidth) {
        return false
    }
    if (this.y <= -this.height || this.y >= Graphics.boxHeight) {
        return false
    }
