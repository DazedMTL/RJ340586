...

{
    ...
    function load() {
        ...
        for (let file of $p['json file']) {
            ...
            loader = loader.add(name, `img/spines/${name}.json`);
        }
    ...
    //ドラゴン行動時のSpineアニメーション＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
    // ドラゴンモーション名 一覧
    // ・scratch　　　　　 →特殊攻撃(爪攻撃) = OK
    // ・dragon_bless　　　→必殺攻撃(ドラゴンブレス)
    // ・roar　　　　　　　→咆哮(カットイン前挙動) = OK
    // ・damage　　　　　　→ダメージ(被弾時)※(setDamageAnimationisDragon)にて設定 = OK
    // ・bite　　　　　　　→通常攻撃(噛みつき攻撃) = OK
    Game_Screen.prototype.setStartAnimationisDragon = function (action) {
        if (SceneManager._scene instanceof Scene_Battle) {
            var sbScene = SceneManager._scene;
            var spSet = sbScene._spriteset;
            for (var i = 0; i < spSet._enemySprites.length; i++) {
                var enemy = spSet._enemySprites[i]._enemy;

                //完全体（Perfect）
                if (spSet._enemySprites[i]._enemy._enemyId == 113) {
                    var childrens = spSet._enemySprites[i].children;
                    for (var j = 0; j < childrens.length; j++) {
                        if (childrens[j] instanceof Sprite_Spine) {


                            // -↓ここです
                            if (action._item._itemId == 1110) childrens[j].spine().setSkeleton('battle').setSkin('Perfect').setAnimation(0, ['bite', 'idle'], 'sequential', 'none');
                            if (action._item._itemId == 1114) childrens[j].spine().setSkeleton('battle').setSkin('Perfect').setAnimation(0, ['scratch', 'idle'], 'sequential', 'none');
                            if (action._item._itemId == 1118) childrens[j].spine().setSkeleton('battle').setSkin('Perfect').setAnimation(0, ['roar', 'idle'], 'sequential', 'none');
                            if (action._item._itemId == 1122) childrens[j].spine().setSkeleton('battle').setSkin('Perfect').setAnimation(0, ['roar', 'idle'], 'sequential', 'none');
                            if (action._item._itemId == 1126) childrens[j].spine().setSkeleton('battle').setSkin('Perfect').setAnimation(0, ['roar', 'idle'], 'sequential', 'none');
                            if (action._item._itemId == 1130) childrens[j].spine().setSkeleton('battle').setSkin('Perfect').setAnimation(0, ['roar', 'idle'], 'sequential', 'none');
                            if (action._item._itemId == 1134) childrens[j].spine().setSkeleton('battle').setSkin('Perfect').setAnimation(0, ['dragon_bless', 'idle'], 'sequential', 'none');
                        }
                    }
                }
                //181
                if (spSet._enemySprites[i]._enemy._enemyId == 181) {
                    var childrens = spSet._enemySprites[i].children;
                    for (var j = 0; j < childrens.length; j++) {
                        if (childrens[j] instanceof Sprite_Spine) {
                            ...
                            // -↓ここです
                            if (action._item._itemId == 1111) childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['bite', 'idle'], 'sequential', 'none');
                            if (action._item._itemId == 1115) childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['scratch', 'idle'], 'sequential', 'none');
                            if (action._item._itemId == 1119) childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['roar', 'idle'], 'sequential', 'none');
                            if (action._item._itemId == 1123) childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['roar', 'idle'], 'sequential', 'none');
                            if (action._item._itemId == 1127) childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['roar', 'idle'], 'sequential', 'none');
                            if (action._item._itemId == 1131) childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['roar', 'idle'], 'sequential', 'none');
                            if (action._item._itemId == 1135) childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['dragon_bless', 'idle'], 'sequential', 'none');
                        }
                    }
                }
                //ゾンビ１（Zombie）
                if (spSet._enemySprites[i]._enemy._enemyId == 190) {
                    var childrens = spSet._enemySprites[i].children;
                    for (var j = 0; j < childrens.length; j++) {
                        if (childrens[j] instanceof Sprite_Spine) {
                ...
                //ゾンビ２（Zombie）
                if (spSet._enemySprites[i]._enemy._enemyId == 191) {
                    var childrens = spSet._enemySprites[i].children;
                    for (var j = 0; j < childrens.length; j++) {
                        if (childrens[j] instanceof Sprite_Spine) {
                            ...
                            // -↓ここです
                            if (action._item._itemId == 1112) childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['bite', 'idle'], 'sequential', 'none');
                            if (action._item._itemId == 1116) childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['scratch', 'idle'], 'sequential', 'none');
                            if (action._item._itemId == 1120) childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['roar', 'idle'], 'sequential', 'none');
                            if (action._item._itemId == 1124) childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['roar', 'idle'], 'sequential', 'none');
                            if (action._item._itemId == 1128) childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['roar', 'idle'], 'sequential', 'none');
                            if (action._item._itemId == 1132) childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['roar', 'idle'], 'sequential', 'none');
                            if (action._item._itemId == 1136) childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['dragon_bless', 'idle'], 'sequential', 'none');
                        }
                    }
                }
                //ゾンビ３（Zombie）
                if (spSet._enemySprites[i]._enemy._enemyId == 192) {
                    var childrens = spSet._enemySprites[i].children;
                    for (var j = 0; j < childrens.length; j++) {
                        if (childrens[j] instanceof Sprite_Spine) {
                            ...
                            // -↓ここです
                            if (action._item._itemId == 1113) childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['bite', 'idle'], 'sequential', 'none');
                            if (action._item._itemId == 1117) childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['scratch', 'idle'], 'sequential', 'none');
                            if (action._item._itemId == 1121) childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['roar', 'idle'], 'sequential', 'none');
                            if (action._item._itemId == 1125) childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['roar', 'idle'], 'sequential', 'none');
                            if (action._item._itemId == 1129) childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['roar', 'idle'], 'sequential', 'none');
                            if (action._item._itemId == 1133) childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['roar', 'idle'], 'sequential', 'none');
                            if (action._item._itemId == 1137) childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['dragon_bless', 'idle'], 'sequential', 'none');
                        }
                    }
                }
            }
    ...
    //ドラゴン被ダメージ時のSpineアニメーション＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
    Game_Screen.prototype.setDamageAnimationisDragon = function () {
        if (SceneManager._scene instanceof Scene_Battle) {
            var sbScene = SceneManager._scene;
            var spSet = sbScene._spriteset;
            for (var i = 0; i < spSet._enemySprites.length; i++) {
                ...
                if (spSet._enemySprites[i]._enemy._enemyId == 113) {
                    ...
                    for (var j = 0; j < childrens.length; j++) {
                        if (childrens[j] instanceof Sprite_Spine) {
                            if (0 == $gameTroop.members()[0].hp) {
                                childrens[j].spine().setSkeleton('battle').setSkin('Perfect').setAnimation(0, ['damage', "idle_dying"], 'continue', 'false');
                                break;
                            }
                            if ($gameTroop.members()[0].enemy().params[0] / 4 > $gameTroop.members()[0].hp) {
                                childrens[j].spine().setSkeleton('battle').setSkin('Perfect').setAnimation(0, ['damage', "idle_dying"], 'continue', 'false');
                                break;
                            }
                            if ($gameTroop.members()[0].enemy().params[0] / 2 > $gameTroop.members()[0].hp) {
                                childrens[j].spine().setSkeleton('battle').setSkin('Perfect').setAnimation(0, ['damage', "idle_craziness"], 'continue', 'false');
                                break;
                            }
                            if (true) {
                                childrens[j].spine().setSkeleton('battle').setSkin('Perfect').setAnimation(0, ['damage', "idle"], 'continue', 'false');
                                break;
                            }
                        }
                    }
                ...
                if (spSet._enemySprites[i]._enemy._enemyId == 181) {
                    ...
                    for (var j = 0; j < childrens.length; j++) {
                        if (childrens[j] instanceof Sprite_Spine) {
                            if (0 == $gameTroop.members()[0].hp) {
                                childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['damage', "idle_dying"], 'continue', 'false');
                                break;
                            }
                            if ($gameTroop.members()[0].enemy().params[0] / 4 > $gameTroop.members()[0].hp) {
                                childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['damage', "idle_dying"], 'continue', 'false');
                            ...
                            if ($gameTroop.members()[0].enemy().params[0] / 2 > $gameTroop.members()[0].hp) {
                                childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['damage', "idle_craziness"], 'continue', 'false');
                                break;
                            }
                            if (true) {
                                childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['damage', "idle"], 'continue', 'false');
                                break;
                            }
                        }
                    }
    ...
    //ドラゴン待機中のSpineアニメーション＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
    // ・idle　　　　　　　→通常待機(体力51％以上) = OK
    // ・idle_craziness　　→発狂状態(体力50％未満) = OK
    // ・idle_dying　　　　→瀕死待機(体力25％以下)= OK
    // ・dead　　　　　　　→敗北(死亡時) = OK
    Game_Screen.prototype.setWaitDragon = function () {
        if (SceneManager._scene instanceof Scene_Battle) {
            var sbScene = SceneManager._scene;
            var spSet = sbScene._spriteset;
            for (var i = 0; i < spSet._enemySprites.length; i++) {
                ...
                if (spSet._enemySprites[i]._enemy._enemyId == 113) {
                    ...
                    for (var j = 0; j < childrens.length; j++) {
                        if (childrens[j] instanceof Sprite_Spine) {
                            ...
                            // 補足：吉原　
                            // $gameTroop.members()[0].hp 敵グループ先頭の敵の現在のHP
                            // $gameTroop.members()[0].enemy().params[0] 敵グループ先頭の敵の最大HP

                            if (0 == $gameTroop.members()[0].hp) {
                                console.log("HP_DIE");
                                childrens[j].spine().setSkeleton('battle').setSkin('Perfect').setAnimation(0, 'dead');
                                break;
                            }
                            if ($gameTroop.members()[0].enemy().params[0] / 4 > $gameTroop.members()[0].hp) {
                                console.log("HP_4");

                                childrens[j].spine().setSkeleton('battle').setSkin('Perfect').setAnimation(0, 'idle_dying');
                                break;
                            }
                            if ($gameTroop.members()[0].enemy().params[0] / 2 > $gameTroop.members()[0].hp) {
                                console.log("HP_2");

                                childrens[j].spine().setSkeleton('battle').setSkin('Perfect').setAnimation(0, 'idle_craziness');
                                break;
                            }
                            if (true) {
                                console.log("HP_FULL");
                                childrens[j].spine().setSkeleton('battle').setSkin('Perfect').setAnimation(0, 'idle');
                                break;
                            }
                        }
                    }
                ...
                if (spSet._enemySprites[i]._enemy._enemyId == 181) {
                    ...
                    for (var j = 0; j < childrens.length; j++) {
                        if (childrens[j] instanceof Sprite_Spine) {
                            ...
                            if (0 == $gameTroop.members()[0].hp) {
                                childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, 'dead');
                                break;
                            }
                            if ($gameTroop.members()[0].enemy().params[0] / 4 > $gameTroop.members()[0].hp) {
                                childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, 'idle_dying');
                                break;
                            }
                            if ($gameTroop.members()[0].enemy().params[0] / 2 > $gameTroop.members()[0].hp) {
                                childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, 'idle_craziness');
                                break;
                            }
                            if (true) {
                                childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, 'idle');
                                break;
                            }
                        }
                    }
    ...
    //ドラゴン死亡時のSpineアニメーション
    Game_Screen.prototype.deadAnimationDragon = function () {
        if (SceneManager._scene instanceof Scene_Battle) {
            var sbScene = SceneManager._scene;
            var spSet = sbScene._spriteset;
            for (var i = 0; i < spSet._enemySprites.length; i++) {
                ...
                if (spSet._enemySprites[i]._enemy._enemyId == 113) {
                    ...
                    for (var j = 0; j < childrens.length; j++) {
                        if (childrens[j] instanceof Sprite_Spine) {
                            childrens[j].spine().setSkeleton('battle').setSkin('Perfect').setAnimation(0, ['dead'], 'sequential', 'none');
                            break;
                        }
                    }
                }
                if (spSet._enemySprites[i]._enemy._enemyId == 181) {
                    ...
                    for (var j = 0; j < childrens.length; j++) {
                        if (childrens[j] instanceof Sprite_Spine) {
                            childrens[j].spine().setSkeleton('battle').setSkin('Zombie').setAnimation(0, ['dead'], 'sequential', 'none');
                            break;
                        }
                    }
                }
    ...
    //敵の行動終了時の処理（Game_Enemy.performActionEnd）に追記
    var _dobby_Game_Enemy_prototype_performActionEnd = Game_Enemy.prototype.performActionEnd;
    Game_Enemy.prototype.performActionEnd = function () {
        if (this._enemyId == 113 && this._motionAction &&
            this._motionAction._item instanceof Game_Item &&
            this._motionAction._item._dataClass == "skill") {
            $gameScreen.setWaitDragon();
            this._motionAction = null;
            this._motionAction = undefined;
        }
        if (this._enemyId == 181 && this._motionAction &&
            this._motionAction._item instanceof Game_Item &&
            this._motionAction._item._dataClass == "skill") {
            $gameScreen.setWaitDragon();
            this._motionAction = null;
        ...
        if (this._enemyId == 190 && this._motionAction &&
            this._motionAction._item instanceof Game_Item &&
            this._motionAction._item._dataClass == "skill") {
            $gameScreen.setWaitDragon();
            this._motionAction = null;
        ...
        if (this._enemyId == 191 && this._motionAction &&
            this._motionAction._item instanceof Game_Item &&
            this._motionAction._item._dataClass == "skill") {
            $gameScreen.setWaitDragon();
            this._motionAction = null;
        ...
        if (this._enemyId == 192 && this._motionAction &&
            this._motionAction._item instanceof Game_Item &&
            this._motionAction._item._dataClass == "skill") {
            $gameScreen.setWaitDragon();
            this._motionAction = null;
        ...
        _dobby_Game_Enemy_prototype_performActionEnd.call(this);
    };

    //敵が被ダメージの時の処理（Game_Enemy.performDamage）に追記
    var _dobby__Game_Enemy_prototype_performDamage = Game_Enemy.prototype.performDamage;
    Game_Enemy.prototype.performDamage = function () {
        if (this._enemyId == 113) {
            $gameScreen.setDamageAnimationisDragon();
        }
        if (this._enemyId == 181) {
            $gameScreen.setDamageAnimationisDragon();
        }
        if (this._enemyId == 190) {
            $gameScreen.setDamageAnimationisDragon();
        }
        if (this._enemyId == 191) {
            $gameScreen.setDamageAnimationisDragon();
        }
        if (this._enemyId == 192) {
            $gameScreen.setDamageAnimationisDragon();
        }
        _dobby__Game_Enemy_prototype_performDamage.call(this);
    }

    //敵が行動を開始する処理（Game_Enemy.performActionStart）に追記
    var _dobby_Game_Enemy_prototype_performActionStart = Game_Enemy.prototype.performActionStart;
    Game_Enemy.prototype.performActionStart = function (action) {
        if (this._enemyId == 113 && action &&
            action._item instanceof Game_Item &&
            action._item._dataClass == "skill") {
            this._motionAction = action;
            $gameScreen.setStartAnimationisDragon(action);
        }
        if (this._enemyId == 181 && action &&
            action._item instanceof Game_Item &&
            action._item._dataClass == "skill") {
            this._motionAction = action;
            $gameScreen.setStartAnimationisDragon(action);
        }
        if (this._enemyId == 190 && action &&
            action._item instanceof Game_Item &&
            action._item._dataClass == "skill") {
            this._motionAction = action;
            $gameScreen.setStartAnimationisDragon(action);
        }
        if (this._enemyId == 191 && action &&
            action._item instanceof Game_Item &&
            action._item._dataClass == "skill") {
            this._motionAction = action;
            $gameScreen.setStartAnimationisDragon(action);
        }
        if (this._enemyId == 192 && action &&
            action._item instanceof Game_Item &&
            action._item._dataClass == "skill") {
            this._motionAction = action;
            $gameScreen.setStartAnimationisDragon(action);
        }
        _dobby_Game_Enemy_prototype_performActionStart.call(this, action);
    };
    ...
    class Sprite_Spine extends Sprite {
        ...
        updateSkin(spine) {
            ...
            let core = PIXI.spine.core || PIXI.spine;
            let skin = new core.Skin('');
        ...
        updateBone(spine) {
            ...
            for (let name in this._bone) {
                ...
                if (bone_p.screen) {
                    [_bone.x, _bone.y] = this.computeScreenVertices([_bone.x, _bone.y]);
                }
                ...
                if (spine.track[id] != this._track[id]) {
                    ...
                    list.forEach((animation, index) => {
                        ...
                        if (index == 0 && track.interrupt) {
                            entry = state.setAnimation(id, animation.name, _loop);
                        } else {
                            entry = state.addAnimation(id, animation.name, _loop, 0);
                        }
        ...
        getAttachmentData(slotName, screen = true) {
            ...
            let core = PIXI.spine.core || PIXI.spine;
            let data = {};
            if (attachment instanceof core.VertexAttachment) {
            ...
            if (attachment instanceof core.ClippingAttachment) {
            ...
            if (attachment instanceof core.PointAttachment) {
        ...
        hitTest(x, y, screen = true) {
            let core = PIXI.spine.core || PIXI.spine;
            let hits = [];
            let bounds = new core.SkeletonBounds();
        ...
        onEvent(entry, event) {
            ...
            let switches = event.data.switches;
            if (switches == null) {
                switches = {};
                let stringValue = event.data.stringValue;
                for (let value of stringValue.replace(/ +/, '').split(/,/)) {
...
// tatsunoko
var dobby_Scene_Battle_prototype_start = Scene_Battle.prototype.start;
Scene_Battle.prototype.start = function () {
    var spSet = this._spriteset;
    for (var i = 0; i < spSet._enemySprites.length; i++) {
        var enemy = spSet._enemySprites[i]._enemy;
        if (enemy._enemyId == 113) {
            let spine = new Game_Spine();
            let sprite = new Sprite_Spine(spine);
            spSet._enemySprites[i].addChild(sprite);
            spine.setSkeleton('battle').setSkin('Perfect').setAnimation(0, 'idle');
            break;
        }
        if (enemy._enemyId == 181) {
            let spine = new Game_Spine();
            let sprite = new Sprite_Spine(spine);
            spSet._enemySprites[i].addChild(sprite);
            spine.setSkeleton('battle').setSkin('Zombie').setAnimation(0, 'idle');
            break;
        }
        if (enemy._enemyId == 190) {
            let spine = new Game_Spine();
            let sprite = new Sprite_Spine(spine);
            spSet._enemySprites[i].addChild(sprite);
            spine.setSkeleton('battle').setSkin('Zombie').setAnimation(0, 'idle');
    ...
    dobby_Scene_Battle_prototype_start.call(this);
};
