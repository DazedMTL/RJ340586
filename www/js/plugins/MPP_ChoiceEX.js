...
(function () {
    ...
    Game_Interpreter.prototype.setupChoices = function (params) {
        ...
        if (data.choices.length > 0) {
            ...
            if (defaultType > 5) {
                // debugger;
                // const _defaultType        = defaultType - 6;
                // const times               = Math.floor(_defaultType / 5) + 1;
                // defaultType               = 10 * times + _defaultType - (_defaultType + 1);
                $gameVariables._data[150] = defaultType;
            ...
            //除外リスト
            if ($gameMap.mapId() === 263) defaultType = data.choices.length - 1; //ID263：回想マップ
            if ($gameSwitches.value(202)) defaultType = 0; //ギルド受付
            if ($gameVariables.value(150) !== 0) defaultType = $gameVariables.value(150);
    ...
    Game_Interpreter.prototype.addChoices = function (params, i, data, d) {
        ...
        for (var n = 0; n < params[0].length; n++) {
            ...
            if (!enable && $gameSwitches.value(3795)) str = "？？？？　　　　　　"; //3795：is回想選択シーン
            if (!enable && $gameSwitches.value(207)) str = "？？？？　　　　　　"; //207：isランク詳細シーン
