...
(function () {
    ...
    Sprite_Minimap.prototype.createObjectSprites = function () {
        this._objectSprites = [];
        // tatsunoko
        this._objectSprites2 = [];
        var object2 = this._objectSprites2;
        //var objects = $gameMap.events().concat($gameMap.vehicles());
        var objects = $gameMap.events();
        ...
        for (var ii = 0; ii < objects.length; ii++) {
            ...
            var pages = obj1.event().pages;

            for (var kk = 0; kk < pages.length; kk++) {
                var tIndex = 0;
                var tCommands = pages[kk].list
                var result = false;
                for (tIndex = 0; tIndex < tCommands.length; tIndex++) {
                    if (tCommands[tIndex] && (tCommands[tIndex].code === 108 || tCommands[tIndex].code === 408)) {
                        var tcomment = tCommands[tIndex].parameters[0];
                        if (Minimap.regex['person'].exec(tcomment) ||
                            Minimap.regex['object'].exec(tcomment) ||
                            Minimap.regex.moveEvent.test(tcomment)) {
                            object2.push(obj1);
                            result = true;
                        }
                    } else {
        ...
        // tatsunoko
        var object3 = object2.concat($gameMap.vehicles());
        object3.forEach(function (obj) {
