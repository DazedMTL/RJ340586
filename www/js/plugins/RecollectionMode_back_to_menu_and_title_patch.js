...
Scene_Recollection.prototype.create = function () {
    ...
    if (SceneManager._stack[sceneStackLen - 1].name === "SpecialMenu_Scene") {
        ...
        if (SceneManager._stack && SceneManager._stack.length > 0) {
            SceneManager._stack.forEach(function (value) {
                Scene_Recollection.returnGameObjects.stack.push(value);
            });
        }
...
Scene_Recollection.prototype.commandBackTitle = function () {
    ...
    if (Scene_Recollection.returnScene === "SpecialMenu_Scene") {
        ...
        SceneManager._stack = Scene_Recollection.returnGameObjects.stack;
        $gameSystem = _system;
        $gameScreen = _screen;
        $gameTimer = _timer;
        ...
        if (sLen > 0 && SceneManager._stack[sLen - 1].name === "SpecialMenu_Scene") {
            exists = true;
        }
        ...
        if (exists) {
            ...
            SceneManager.goto(GLOBAL_SpecialMenuScene.scene);
        }
    } else {
...
Scene_Recollection.prototype.commandDoRecMode = function () {
    ...
    if (this._rec_list.is_valid_picture(this._rec_list.index() + 1)) {
        ...
        if (this._mode === "recollection") {
            ...
            // this.fadeOutAll()の中身を改変
            var time = this.fadeSpeed();
...
Window_RecList.prototype.get_global_variables = function () {
    ...
    for (var i = 1; i <= maxSaveFiles; i++) {
        ...
        // this._global_variables["switches"][3801] = true;
        // this._global_variables["switches"][3802] = true;
        // tatsunoko
