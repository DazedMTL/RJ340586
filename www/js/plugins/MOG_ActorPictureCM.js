...
Actor_CMPicture.prototype.createCM2 = function () {
    ...
    // tatsunoko
    this._bust.scale.x = 0.607;
    this._bust.scale.y = 0.607;
...
Actor_CMPicture.prototype.refreshBitmapCM2 = function () {
    ...
    // Mod tatsunoko
    var actorId = this.fileName().split("_");
    // this._bust.bitmap = ImageManager.loadActorpicCM(fileName);
    //this._bust.bitmap = getCharCosplayImgByActorId(actorId[1]);
    let hp = $gameActors.actor(actorId[1]).hp;
    let mhp = $gameActors.actor(actorId[1]).param(0);

    this._bust.bitmap = getBattleImgByActorId(actorId[1], Math.floor(hp / mhp * 100));

    // tatsunoko
    this._bust.anchor.x = 0;
    this._bust.anchor.y = 0;
    this._bust.scale.x = 0.607;
    this._bust.scale.y = 0.607;
    this._bust.anchor.x = 0.5;
    this._bust.anchor.y = 1.0;
...
Actor_CMPicture.prototype.getDataCM2 = function () {
    ...
    //     0, 0, 0];
    var yPos = this._actor ? ImageManager.getActorBattleImgAdjustY(this._actor._actorId) : 100;
    this._bustData = [this._bust.bitmap.width, this._bust.bitmap.height,
    Moghunter.actor_cm2_x, Number(Moghunter.actor_cm2_y + yPos), 0, 0, true,
    Moghunter.actor_cm2_slideX, Moghunter.actor_cm2_slideY,
        0, 0, 0];
    ...
    this._bustData[3] = Number(Moghunter.actor_cm2_y + yPos) + Graphics.boxHeight;
...
Actor_CMPicture.prototype.updateBreathEffect = function () {
    ...
    this._bust.scale.y = 0.607 + this._bust.zoom.y;
};
