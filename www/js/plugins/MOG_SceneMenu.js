...
Scene_Menu.prototype.createCommands = function () {
    ...
    // Add tatsunoko
    var halfLength = Math.floor(this._comList.length / 2);
    var posXcnt = 0;
    ...
    for (var i = 0; i < this._comList.length; i++) {
        ...
        const interval = 76;
        if (halfLength > i) {
            this._compos[i] = [Moghunter.scMenu_ComX + (interval * posXcnt), Moghunter.scMenu_ComY];
        } else {
            this._compos[i] = [Moghunter.scMenu_ComX + (interval * posXcnt + 38), Moghunter.scMenu_ComY + 60];
        }
...
Scene_Menu.prototype.createCommandName = function () {
    this._commandName = new Sprite(new Bitmap(150, 32));
    this._commandName.bitmap.fontSize = 24;
...
Scene_Menu.prototype.updateCommandName = function () {
    ...
    ;
    this._commandName.x = this.commandMoveTo(this._commandName.x, Moghunter.scMenu_ComNameX - 25);
    this._commandName.y = this.commandMoveTo(this._commandName.y, Moghunter.scMenu_ComNameY);
    this._commandName.opacity = 255;
...
Scene_Menu.prototype.refreshCommandName = function () {
    ...
    this._commandName.bitmap.drawText(this._comList[this._commandNameIndex].name, 0, 0, 150, 32, "center")
...
Scene_Menu.prototype.update = function () {
    ...
    ;
    if (!this._selection && this._facesBitmaps && this._facesBitmaps[0].isReady()) {
        this.createAfter()
    }
    ;
    if (this._selection) {
...
MBustMenu.prototype.posX = function () {
    var space = Math.floor((Graphics.boxWidth - 32 * 2) / this._maxMembers);
...
MBustMenu.prototype.createCharaters = function () {
    ...
    this._char = new Sprite(getCharCosplayImg(this._actor));

    this._char.scale.x = 0.507;
    this._char.scale.y = 0.507;
    this._char.anchor.x = 0.5;
    this._char.anchor.y = 1.0;
    this._char.x = this.posX() + Moghunter.scMenu_CharX - 12;
    this._char.y = Graphics.boxHeight + Moghunter.scMenu_CharY;
    // add tatsunoko
    this.adjustPostionChar(this._actor._actorId);
...
// add tatsunoko
MBustMenu.prototype.adjustPostionChar = function (actorId) {
    if (this._char) {
        switch (actorId) {
            case 1:
                this._char.y = this._char.y - 80;
                break;
            case 2:
                this._char.y = this._char.y - 150;
                break;
            case 3:
                this._char.y = this._char.y - 0;
                break;
            case 4:
                this._char.y = this._char.y - 60;
                break;
        }
        ;
    }
...
// * create SP,AP
//==============================
Moghunter.scMenu_SpX = 0;
Moghunter.scMenu_SpY = 164;
MCharStatus.prototype.createSPNumber = function () {
    this._sp = new Sprite(new Bitmap(140, 40));
    this._sp.x = this._layout.x + Moghunter.scMenu_SpX;
    this._sp.y = this._layout.y + Moghunter.scMenu_SpY;
    this._sp.bitmap.fontSize = 22;
    this._sp.bitmap.outlineColor = 'rgba(0, 0, 0, 0.8)';
    this._sp.bitmap.outlineWidth = 5;
    this._sp.bitmap.fontFace = 'GameFont2';
    this._sp.bitmap.textColor = 'rgb(92,239,226)';
    let sp = 0;
    switch (this._actor.name()) {
        case "クレア":
            sp = $gameVariables.value(74);
            break;
        case "ノラ":
            sp = $gameVariables.value(78);
            break;
        case "リルム":
            sp = $gameVariables.value(76);
            break;
        case "ソフィア":
            sp = $gameVariables.value(80);
            break;
    }
    if (sp === 0) this._sp.bitmap.textColor = 'rgb(160,160,160)';
    // console.log(this._sp.bitmap.fontSize,this._sp.bitmap.fontFace);
    this._sp.bitmap.drawText(sp, 0, 0, 120, 32, "center");
    // this._sp.scale.y = 0.9;
    this.addChild(this._sp);
};

Moghunter.scMenu_ApX = 63;
Moghunter.scMenu_ApY = 164;
MCharStatus.prototype.createAPNumber = function () {
    this._ap = new Sprite(new Bitmap(140, 40));
    this._ap.x = this._layout.x + Moghunter.scMenu_ApX;
    this._ap.y = this._layout.y + Moghunter.scMenu_ApY;
    this._ap.bitmap.fontSize = 22;
    this._ap.bitmap.outlineColor = 'rgba(0, 0, 0, 0.8)';
    this._ap.bitmap.outlineWidth = 5;
    this._ap.bitmap.fontFace = 'GameFont2';
    this._ap.bitmap.textColor = 'rgb(255,102,204)';
    let ap = 0;
    switch (this._actor.name()) {
        case "クレア":
            ap = $gameVariables.value(75);
            break;
        case "ノラ":
            ap = $gameVariables.value(79);
            break;
        case "リルム":
            ap = $gameVariables.value(77);
            break;
        case "ソフィア":
            ap = $gameVariables.value(81);
            break;
    }
    if (ap === 0) this._ap.bitmap.textColor = 'rgb(160,160,160)';
    this._ap.bitmap.drawText(ap, 0, 0, 120, 32, "center");
    // this._ap.scale.y = 0.9;
    this.addChild(this._ap);
};
...
MCharStatus.prototype.createHPMeter = function () {
    ...
    this._hpMeter.y = this._layout.y + Moghunter.scMenu_HPMeterY + ALT_adjustY;
...
MCharStatus.prototype.createMPMeter = function () {
    ...
    this._mpMeter.y = this._layout.y + Moghunter.scMenu_MPMeterY + ALT_adjustY;
...
MCharStatus.prototype.createHPNumber = function () {
    ...
    var x = this._layout.x + Moghunter.scMenu_HPNumberX + 10;
    var y = this._layout.y + Moghunter.scMenu_HPNumberY + ALT_adjustY;
    for (var i = 0; i < 5; i++) {
...
MCharStatus.prototype.createHPNumber2 = function () {
    ...
    var y = this._layout.y + Moghunter.scMenu_HPNumber2Y + ALT_adjustY;
    for (var i = 0; i < 5; i++) {
...
MCharStatus.prototype.createMPNumber = function () {
    ...
    var x = this._layout.x + Moghunter.scMenu_MPNumberX + 10;
    var y = this._layout.y + Moghunter.scMenu_MPNumberY + ALT_adjustY;
    for (var i = 0; i < 5; i++) {
...
MCharStatus.prototype.createMPNumber2 = function () {
    ...
    var y = this._layout.y + Moghunter.scMenu_MPNumber2Y + ALT_adjustY;
    for (var i = 0; i < 5; i++) {
...
MCharStatus.prototype.createLayoutStatus = function () {
    ...
    this._layout.x = this.posX() - Moghunter.scMenu_CharX - 20 - 25;
...
Window_MenuCommand.prototype.dcursorDown = function (wrap) {
    ...
    // Add tatsunoko
    if ((maxItems - 1) !== index) {
        if (this.isDobbyUpper()) {
            this.select((index + maxCols) % maxItems);
        } else {
            this.select((index - (maxCols - 1)));
        }
    } else {
        this.select(0);
    }
...
Window_MenuCommand.prototype.dcursorUp = function (wrap) {
    var index = this.index();
    var maxItems = this.maxItems();
    var maxCols = this.maxCols();
...
Window_MenuCommand.prototype.dcursorRight = function (wrap) {
    var index = this.index();
    var maxItems = this.maxItems();
    var maxCols = this.maxCols();
    // del tatsunoko
...
Window_MenuCommand.prototype.dcursorLeft = function (wrap) {
    var index = this.index();
    var maxItems = this.maxItems();
    var maxCols = this.maxCols();
    // Del tatsunoko
...
Window_MenuCommand.prototype.maxCols = function () {
    return Math.floor(this.maxItems() / 2);
};

// Add tatsunoko
Window_MenuCommand.prototype.isDobbyUpper = function () {
    var index = this.index();
    var maxItems = this.maxItems();
    var maxCols = this.maxCols();

    return (maxCols >= 2 && (index < maxCols));
};
