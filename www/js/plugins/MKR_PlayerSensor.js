...
(function () {
        ...
        if (this.getSensorStatus() == 1 && (!this.isStarting() || this.getActiveMode() == 1)) {
            ...
            if (this.isFoundPlayer() && this.getForceLost() == 0) {
                ...
                if ($gameSwitches.value(306)) {
                    this._lostDelayCount++;
                    if (this._lostDelayCount >= 12) {
    ...
    Game_Event.prototype.isFoundPlayer = function () {
        ...
        switch (this.getSensorType()) {
            case "l": // 直線の探索
                ...
                if ($gameSwitches.value(306) && !$gameSwitches.value(307)) return false;
                break;
            case "f": // 扇範囲の探索
    ...
    const hasPsensorRegExp = /Psensor/
    const _GameEvent_lock = Game_Event.prototype.lock;
    Game_Event.prototype.lock = function () {
        const ev = $dataMap.events[this._eventId];
        let hasPsensor = false;
        if (!!ev && !!ev.meta) {
            const keys = Object.keys(ev.meta);
            for (let i = 0; i < keys.length; i++) {
                if (hasPsensorRegExp.test(keys[i])) {
                    hasPsensor = true;
                    i = keys.length;
                }
            }
        }
        if (this.getSensorStatus() != 1 && !hasPsensor) {
            _GameEvent_lock.call(this);
        } else {

            if (!this._locked) {  // 話しかけられた場合振り向かない(探索者が探索中に限る)
