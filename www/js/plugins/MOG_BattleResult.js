...
Scene_Battle.prototype.updateWindowPositions = function () {
    ...
    if ($gameTemp._battleEnd) {
        this._statusWindow.opacity -= 20;
        this._statusWindow.contentsOpacity -= 20;
    }
    ;
};
...
BattleResult.prototype.addIcon = function (sprite, data) {
    ...
    var name = new Sprite(new Bitmap(320, 32));
    name.bitmap.drawText(data.name, 0, 0, 320, 32);
    name.x = w + 4;
...
BattleResult.prototype.update = function () {
    ...
    if (this._numbeData[0] === -1 && this._number_img1.isReady() && this._number_img2.isReady()) {
        this.getData()
    }
...
BattleResult.prototype.updateLayoutAnimation = function () {
    ...
    if (this._layout.scale.x > 1.00) {
        this._layout.scale.x -= 0.2;
        if (this._layout.scale.x < 1.00) {
            this._layout.scale.x = 1.00
        }
        ;
    } else {
        this.phaseAniClear();
...
BattleResult.prototype.updateVictAnimation = function () {
    if ($gameSwitches.value(210)) { //210:Battle:Vicスキップ
...
BattleResult.prototype.updateExp = function () {
    ...
    var dif_number = this.update_dif(this._expOld, $gameTemp._bResult[1], 10);
    if (this._expOld != dif_number || $gameTemp._bResult[1] === 0) {
...
BattleResult.prototype.updateGold = function () {
    ...
    var dif_number = this.update_dif(this._goldOld, $gameTemp._bResult[2], 10);
    if (this._goldOld != dif_number || $gameTemp._bResult[1] === 0) {
...
BattleResult.prototype.createActorSprite = function () {
    ...
    var img = getCharCosplayImg(this._actor);
    this._spriteActor = new Sprite(img);

    // mod tatsunoko
    this._spriteActor.scale.x = 0.607;
    this._spriteActor.scale.y = 0.607;
...
BattleResult.prototype.updateLevelAnimation = function () {
    ...
    this._spriteActor.opacity += 40;
    this._levelLayout.opacity += 40;
    this._name.opacity += 40;
    this._par.opacity += 40;
    if (this._skill) {
...
BattleResult.prototype.updateLevelPosition = function () {
    ...
    this._spriteActor.x = this.sprite_move_to(this._spriteActor.x, Moghunter.bresult_actor_x, 15);
    // mod tatsunoko
    //this._spriteActor.y = Graphics.boxHeight - this._spriteActor.height + Moghunter.bresult_actor_y;
    this._spriteActor.y = Graphics.boxHeight - this._spriteActor.height + Moghunter.bresult_actor_y + ImageManager.getActorBattleResultImgAdjustY(this._actor._actorId);
    if (this._levelLayout.scale.x > 1.00) {
        this._levelLayout.scale.x -= 0.075;
        if (this._levelLayout.scale.x < 1.00) {
            this._levelLayout.scale.x = 1.00
        }
    }
    this._par.x = this.sprite_move_to(this._par.x, Moghunter.bresult_par0_x, 10);
