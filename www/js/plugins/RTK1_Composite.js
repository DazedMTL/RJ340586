 ...
 * @default 1
 *
 * @param scenarioGoFlag
 * @default 0
 * @type switch
 * @desc 冒険者ギルドの雑用を進めるためのスイッチ。（タツノコ改修）
 * 
 * @help
...
(function (_global) {
	...
	RTK.onReady(function () {
		...
		RTK.text("Weapon Composite Shop", "鍛冶師エレロの工房");
	...
	// add tatsunoko
	var paramSwitch = Number(param['scenarioGoFlag'] || "0");
	...
	Scene_CompositeMenu.prototype.create = function () {
		...
		this._exeWindow.setHandler('ok', function () {
			...
			// Add tatsunoko
			if ($gameVariables.value(504) == 10) {
				$gameSwitches.setValue(paramSwitch, true);
			}
