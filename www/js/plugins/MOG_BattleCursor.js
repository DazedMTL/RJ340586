...
BattleCursor.prototype.updateTouchSelection = function () {
    if (TouchInput.isTriggered() && !SceneManager._scene._actorCommandWindow.active && !SceneManager._scene._skillWindow.active) {
        ...
        for (var i = 0; i < this.actors().length; i++) {
            if (this.isTouchOnTarget(this.actors()[i], this.actors()[i]._battler, 1)) {
                $gameTemp._arrowTarget[1] = this.actors()[i]._battler
            }
            ;
            if (Imported.MOG_BattleHud && this.isTouchOnTargetFace(this.actors()[i], this.actors()[i]._battler, 1)) {
                $gameTemp._arrowTarget[1] = this.actors()[i]._battler
            }
            ;
        }
...
BattleCursor.prototype.isTouchOnTarget = function (sprite, battler, type) {
    ...
    if (sprite.bitmap) {
        ...
        if (TouchInput.x < this.field().x + sprite.x - (sprite._mainSprite.width / 2)) {
            return false
        }
        if (TouchInput.x > this.field().x + sprite.x + (sprite._mainSprite.width / 2)) {
            return false
...
BattleCursor.prototype.update_arrow = function (sprite, target, battler, type) {
    ...
    if (battler._actorId === 3) this._arrow_pos[type][1] += 65;
...
Window_BattleActor.prototype.initialize = function (x, y) {
    ...
    if (String(Moghunter.bcursor_window) === "true" || (!$gameSystem.isSideView() && !Imported.MOG_BattleHud)) {
